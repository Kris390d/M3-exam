...
y_train_data8 = y_train_data5 %>% as.factor() %>% unlist
y_train_data6 = y_train_data5 %>% as.factor() %>% unclass() %>% as.numeric()
y_train_data7 = y_train_data6 %>% as.data.frame() 

y_train_data7 = y_train_data7 %>% mutate(ty = y_train_data7$.) %>% mutate(index = seq(1,37254,1))

y_train_data7 = y_train_data7 %>% rename(sent = ty ) %>% select(sent, index) %>% group_by(index) %>%  count(sent) %>% pivot_wider(names_from = sent, values_from = n, values_fill = 0) %>%  ungroup() %>%  select(2,3,4,5) %>%  as.matrix()
...
```{r}
y_train_data= train_data5 %>% select('y') %>% mutate(y= recode(y, "joy" = 0, "sadness" = 1,"fear" =2, "trust"= 3)) %>% as.matrix()

y_test_data= test_data5 %>% select('y') %>% mutate(y= recode(y, "joy" = 0, "sadness" = 1,"fear" =2, "trust"= 3)) %>% as.matrix()
```
...
sequences <- texts_to_sequences(tokenizer, x_train_data5)

tokenizer2 <- text_tokenizer(num_words = 10000) %>%
  fit_text_tokenizer(x_test_data5)
sequences2 <- texts_to_sequences(tokenizer2, x_test_data5)
```
...
x_test <- sequences2 %>%  vectorize_sequences(dimension = 10000)
```
...
#for training data
tokenizer_train5 <- text_tokenizer(num_words = 5000,
                                  filters = "!\"#$%&()*+,-./:;<=>?@[\\]^_`{|}~\t\n" ) %>%
  fit_text_tokenizer(x_train_data5)

sequences_train5 = texts_to_sequences(tokenizer_train5, x_train_data5)


#For test data

tokenizer_test5 <- text_tokenizer(num_words = 5000,
                                 filters = "!\"#$%&()*+,-./:;<=>?@[\\]^_`{|}~\t\n" ) %>%
  fit_text_tokenizer(x_test_data5)

sequences_test5 = texts_to_sequences(tokenizer_test5, x_test_data5)
...
```{r}
x_train5 <- sequences_train5 %>% vectorize_sequences(dimension = 5000) 
x_test5 <- sequences_test5 %>% vectorize_sequences(dimension = 5000) 
...
```{r}
metrics10 = model5 %>% evaluate(x_test, one_hot_test_labels); metrics10
```
