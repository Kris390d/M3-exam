...
model_lstm2 <- keras_model_sequential() %>%
  layer_embedding(input_dim = ncol(x_train), output_dim = 8) %>%
  layer_lstm(units = 8, dropout = 0.25, recurrent_dropout = 0.25, return_sequences = FALSE) %>%
  layer_dense(units = ncol(one_hot_train_labels), activation = "softmax")
...
#layer_lstm(units = 8,return_sequences = TRUE) %>%
#bidirectional(layer_lstm(units = 8)) %>%
