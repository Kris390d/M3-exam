...
```{r}
metrics3 = model_rnn %>% evaluate(x_test, y_test_data); metrics3
```
Running our model on the test data shows an accuracy of 60%, but we will now try to fine tune it to make it better.

#### Tunning the model

This time we again try to reduce the number of parameters to prevent over fitting. We also make another rnn layer 
```{r}
model_keras2 <- keras_model_sequential()

model_rnn2 <- model_keras2 %>%
  layer_embedding(input_dim = 5000, output_dim = 16) %>%
  layer_simple_rnn(units = 16, return_sequences = TRUE, activation = "tanh" ) %>%
  layer_simple_rnn(units = 16, return_sequences = FALSE, activation = "tanh") %>%
  layer_dense(units = 1, activation = "sigmoid")
```
