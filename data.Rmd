...
Stemming: We use the hunspell package which seems to produce the best stemming for our data. Reducing a word to its “root” word. 
```{r}
library(hunspell)
text_genre_tidy %>%
...
```{r}
labels_words %>%
mutate(word = reorder_within(word, by = tf_idf, within = Pop)) %>%
